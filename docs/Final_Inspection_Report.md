...
4.  **Frontend Dashboard:** A **React/Vite** web interface for researchers to create tasks and monitor simulation progress in real-time.
...
We simulated a **25% Malicious Client** scenario where attackers attempted to destroy the model using "Label Flipping" and "Gradient Scaling" attacks. We compared standard `FedAvg` against our `Trimmed Mean` defense.
